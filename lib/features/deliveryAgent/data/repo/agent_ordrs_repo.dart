...
class AgentOrdrsRepo {
  ...
  Future<ApiResult<AcceptOrdersModel>>
  acceptOrderRestaurant({required String orderId}) async {
    try {
      final res = await apiServices.acceptOrderRestaurant(orderId,{});
      return ApiResult.success(res);
    } catch (e) {
  ...
  Future<ApiResult<AcceptOrdersModel>>
  readyForPickup({required String orderId}) async {
    try {
      final res = await apiServices.readyForPickUp(orderId,{});
      return ApiResult.success(res);
    } catch (e) {
      print('Error in accept order restaurnat : $e');
      return ApiResult.failure(ApiErrorHandler.handle(e));
